...
import { ResponsiveContainer as Container, Legend, type LegendProps, Tooltip } from 'recharts'
...
function Style({ id, config }: StyleProps) {
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'grid min-w-40 items-start gap-1.5 rounded-lg border border-border/60 bg-overlay px-2.5 py-1.5 text-xs text-overlay-fg shadow-xl',
          className,
        )}
      >
...
function resolvePayloadConfig(config: ChartConfig, payload: unknown, key: string) {
